...
from dev_db.utils import get_max_id, model_name, model_has_id_primary_key, hash_instance
...
class DevDBCreator:
    ...
    def get_models(self):
        ...
        for m, n in all_models:
            ...
            if (
                all(map(lambda x: x not in m._meta.db_table, excluded))
                and not m._meta.proxy
            ):
                try:
                    m.objects.first()  # trigger potential database errors
